...
function App() {
  ...
  { 
     if ( prevValue.workDescription === Info.workDescription && Info.workDescription.includes('ДП - Пальне відсутнє.') )
     {
      console.log('q');
      const templateParams = {
        message: `${Info.name} Є дизпаливо по картках`,
    };
      // emailjs.send('service_78bgvog', 'template_rrocfu4', `${Info.name} Є дизпаливо по картках`, 'CIwkzASVn2LRgUqgZ')
      // .then((result) => { 
      // }, (error) => {
      //     console.log(error.text);
      // });
